  ...
  status(fs) {
    ...
    if (stagedFiles.length > 0) {
      ...
      for (const f of stagedFiles) {
        ...
        lines.push(`\t<span class="color-green">${isNew ? 'new file' : 'modified'}:   ${f}</span>`);
      }
      lines.push('');
    }
    ...
    if (modified.length > 0) {
      ...
      for (const m of modified) {
        lines.push(`\t<span class="color-red">${m.status}:   ${m.path}</span>`);
      }
      lines.push('');
    }
    ...
    if (untracked.length > 0) {
      ...
      for (const f of untracked) {
        lines.push(`\t<span class="color-red">${f}</span>`);
      }
      lines.push('');
    }
